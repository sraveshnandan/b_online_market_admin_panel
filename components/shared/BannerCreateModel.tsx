...
const BannerCreateModel = ({ handleNewBannerCreate }: Props) => {
    ...
    const handleBannerCreate = async (ev: FormEvent<HTMLFormElement>) => {
        ...
        if (bannerName.length === 0) {
            ...
            return toast.error("Banner name can't be empty.")
        ...
        try {
            ...
            const res = await fetch("https://bom-api-1-0-1.onrender.com/api/v1/banner", {
                method: "POST",
                headers: {
    ...
    return (
        <Dialog open={open} onOpenChange={setopen}>
            ...
            <DialogContent>
                <DialogHeader className="text-gray-600 font-semibold text-xl">Creating a new banner</DialogHeader>
                <DialogDescription className="p-2 bg-[#efeff1] rounded-md flex flex-col ">
                    ...
                    <form onSubmit={handleBannerCreate} >
                        ...
                        <div className="flex flex-col">
                            <label htmlFor="name" className="mb-1 text-sm font-semibold">Banner name:</label>
                            <input onChange={(ev) => setbannerName(ev.target.value)} className="p-2 rounded-md text-md font-semibold" id="name" type="text" placeholder="banner name" />
                        </div>
...
export default BannerCreateModel